...
class VertexAIModelGarden(_VertexAIBase, BaseLLM):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        return self._normalize_response(prompts, self._parse_response(response), stop=stop, run_manager=run_manager)
        ...
        **kwargs: Any,
        ...
        for chunk in self._normalize_generation(chunks):
          if final_chunk is None:
              final_chunk = GenerationChunk(text=chunk.text, generation_info=chunk.generation_info)
          else:
              final_chunk += GenerationChunk(text=chunk.text, generation_info=chunk.generation_info)
          if run_manager:
              run_manager.on_llm_new_token(
    ...
    def _normalize_generation(self, chunks: List[Generation]) -> Iterator[GenerationChunk]:
        return map(lambda x: GenerationChunk(text=x.text, generation_info=x.generation_info), chunks)

    def _strip_generation_context(self, prompt: str, chunks: List[GenerationChunk]) -> List[GenerationChunk]:
    ...
    def _format_generation_context(self, prompt: str, chunks: List[GenerationChunk]) -> List[GenerationChunk]:
